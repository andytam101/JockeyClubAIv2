...
def convert_dataset(data_x, data_y, target_keys):
    ...
    result_x = np.zeros((total_size, 48), dtype=np.float32)
...
def main():
    data_x = np.load("data/data_x.npz")
    data_y = np.load("data/data_y.npz")
    horse_nums = np.load("data/horse_nums.npz")
    top_3 = np.load("data/top_3.npz")
    ...
    w, p, t = test_accuracy(model, data_x, horse_nums, top_3, train_mean, train_std)
    print("=" * 100)
