...
from database import init_engine, get_session, Horse, Race, Participation
...
def save(directory, x, y, metadata):
...
def main():
    ...
    session = get_session()
    x, y, metadata = load_from_db(session, start_date, end_date)
    session.close()

    # x and y should be dictionary
    save(save_path, x, y, metadata)
