...
class ParticipationRankingLoader(Loader):
    def __init__(self):
        ...
        self.opponent_ps_summary_features = 7

    @property
    def input_features(self):
        return INDIVIDUAL_FEATURES + self.opponent_data_features + self.opponent_ps_summary_features
    ...
    def get_opponent_participations_summary(session, ps: [Participation]):
        ...
        for p in ps:
            ...
            if len(horse_ps) == 0:
                continue
            speed_mean.append(get_group_speed(horse_ps)[0])
            ranking_median.append(get_group_ranking(horse_ps)[0])
            win_odds_mean.append(get_group_win_odds(horse_ps)[0])
            top_1_ratio.append(get_group_top_ratio(horse_ps, 1))
            top_2_ratio.append(get_group_top_ratio(horse_ps, 2))
            top_3_ratio.append(get_group_top_ratio(horse_ps, 3))
            top_4_ratio.append(get_group_top_ratio(horse_ps, 4))

        return [
            np.mean(speed_mean),
            np.mean(ranking_median),
            np.mean(win_odds_mean),
            np.mean(top_1_ratio),
            np.mean(top_2_ratio),
            np.mean(top_3_ratio),
            np.mean(top_4_ratio)
